...
<template>
  <div class="documents-table">
    ...
    <div v-else class="overflow-x-auto border border-gray-200 rounded-lg">
      <table class="min-w-full divide-y divide-gray-200" style="display: table;">
        <thead class="bg-gray-50">
          <tr>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Document</th>Documents
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Action</th>
            <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">History</th>
          </tr>
        </thead>
          ...
          <tr v-for="req in requirements" :key="req.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex items-center">
                <div :title="req.currentStatus" :class="getStatusInfo(req.currentStatus).class" class="w-2 h-8 rounded-full mr-4 flex-shrink-0"></div>
                <div>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              <div v-if="req.currentStatus !== 'not_applicable'">
                <div v-if="req.documentType.is_time_sensitive" class="flex items-center space-x-2">
                  <input type="date" v-model="submissionData[req.documentTypeId].issueDate" class="form-input rounded-md border-gray-300 shadow-sm focus:border-brand-blue-300 focus:ring focus:ring-brand-blue-200 focus:ring-opacity-50 w-full">
                  <button @click="submitDocument(req.documentTypeId)" class="btn-primary whitespace-nowrap" :disabled="!submissionData[req.documentTypeId].issueDate">Submit</button>
                </div>
                <div v-else>
                  <button @click="submitDocument(req.documentTypeId)" class="btn-secondary">Mark as Received</button>
                </div>
              </div>
            </td>
