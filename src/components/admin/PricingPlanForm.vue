    ...
    <div class="space-y-4">
      ...
      <div v-for="(tier, index) in editablePlan.tiers" :key="index" class="p-4 border rounded-md space-y-2 relative">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-medium">Min Estates</label>
            <input type="number" v-model.number="tier.min_estates" required min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
          </div>
          <div>
            <label class="block text-sm font-medium">Max Estates</label>
            <input type="number" v-model.number="tier.max_estates" :min="tier.min_estates" placeholder="No limit" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
          </div>
          <div>
            <label class="block text-sm font-medium">Rate per Estate</label>
            <input type="number" v-model.number="tier.rate_per_estate" required min="0" step="0.01" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
          </div>
        </div>
      </div>
...
const createFreshPlan = () => ({
  ...
  end_date: null,
  tiers: [{ min_estates: 1, max_estates: null, rate_per_estate: 0 }]
});
...
const addTier = () => {
  ...
  const newMin = (lastTier && lastTier.max_estates) ? lastTier.max_estates + 1 : 1;
  editablePlan.value.tiers.push({ min_estates: newMin, max_estates: null, rate_per_estate: 0 });
};
