<template>
  <div>
    ...
    <div v-else class="bg-yellow-50 text-yellow-800 p-6 rounded-2xl border border-yellow-200">
      ...
      <p class="text-sm mt-1">The system requested a layout named <strong>"{{ targetTemplateName }}"</strong>, but it doesn't exist in the library.</p>
...
// 1. Import Workspace Templates
import TemplateStandard from './workspaces/TemplateStandard.vue';
import TemplateEstateStandard from './workspaces/TemplateEstateStandard.vue';
import TemplateUnifiedStandard from './workspaces/TemplateUnifiedStandard.vue';
...
const targetTemplateName = ref('');

// 2. Map available workspace templates
const templateMap = {
  'TemplateUnifiedStandard': TemplateUnifiedStandard,
...
const fetchCaseAndLoadTemplate = async () => {
  ...
  try {
    ...
    // 3. Check for explicitly defined template on FileType
    const explicitTemplate = caseData.fileType?.workspaceTemplate || 
    ...
    // Otherwise, fall back to the standard layout.
      const resolvedTemplateName = caseData.product?.workspace_template || 
                               caseData.product?.workspaceTemplate || 
                               'TemplateUnifiedStandard';

    targetTemplateName.value = resolvedTemplateName;
    
    // 5. Assign the component
    if (templateMap[resolvedTemplateName]) {
      currentTemplateComponent.value = templateMap[resolvedTemplateName];
    } else {
      currentTemplateComponent.value = templateMap['TemplateUnifiedStandard'];
    }
...
// Re-fetch if user switches product slug or case ID
watch(
