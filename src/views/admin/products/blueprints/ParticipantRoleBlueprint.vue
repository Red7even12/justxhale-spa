      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Group on Docs</th>
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          <tr v-for="role in roles" :key="role.id" class="hover:bg-indigo-50/30 transition-colors">
            ...
            <td class="px-6 py-4 text-center">
              <span :class="(role.groupOnDocuments || role.group_on_documents) ? 'bg-blue-100 text-blue-800 border-blue-200' : 'bg-gray-100 text-gray-400 border-gray-200'" class="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider border">
            ...
            <td class="px-6 py-4 text-center">
              <span :class="(role.isActive ?? role.is_active) ? 'bg-green-100 text-green-800 border-green-200' : 'bg-red-100 text-red-800 border-red-200'" class="px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest border">
      ...
      <div class="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <form @submit.prevent="save" class="space-y-4">
          <div>
            ...
            <input v-model="form.name" type="text" required class="w-full border-gray-300 rounded-lg shadow-sm" placeholder="e.g. Company Shareholder">
          </div>

          <div>
            <label class="block text-xs font-black text-gray-500 uppercase mb-1">System Key (Unique)</label>
            <input v-model="form.role_key" :disabled="!!form.id" type="text" required class="w-full border-gray-300 rounded-lg shadow-sm font-mono text-sm disabled:bg-gray-100" placeholder="e.g. company_shareholder">
          ...
          <div class="p-3 bg-blue-50/50 rounded-lg border border-blue-100 flex items-start gap-3">
            <input v-model="form.group_on_documents" type="checkbox" id="group_docs" class="mt-0.5 h-4 w-4 text-indigo-600 border-gray-300 rounded">
            <div>
