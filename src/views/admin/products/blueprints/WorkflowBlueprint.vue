<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 h-[calc(100vh-280px)]">
      ...
      <div class="lg:col-span-1 bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
      ...
      <div class="lg:col-span-3 h-full overflow-hidden">
        <div v-if="!selectedDefinition" class="flex flex-col items-center justify-center h-full bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <p class="text-gray-400 text-sm">Select a workflow definition to manage its logic.</p>
        </div>
    ...
    <div v-if="isDefModalOpen" class="fixed inset-0 bg-gray-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
        <h2 class="text-lg font-black text-gray-900 border-b pb-3">{{ defForm.id ? 'Edit' : 'Create' }} Workflow Definition</h2>
        
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-black text-gray-600 uppercase mb-1">Workflow Name</label>
            <input v-model="defForm.name" type="text" class="w-full border-gray-300 rounded-lg text-sm" placeholder="e.g. Standard Estate Liquidation">
          </div>

          <div v-if="!isFoundryContext">
            <label class="block text-xs font-black text-gray-600 uppercase mb-1">Niche Context (Optional)</label>
            <select v-model="defForm.file_type_id" class="w-full border-gray-300 rounded-lg text-sm">
          ...
          <div>
            ...
            <textarea v-model="defForm.description" rows="3" class="w-full border-gray-300 rounded-lg text-sm"></textarea>
          </div>
...
const apiUrl = computed(() => {
  return isFoundryContext.value
    ? `admin/file-types/${props.fileTypeId}/workflow-definitions`
    : `admin/products/${props.slug}/workflow-definitions`;
});

const loadData = async () => {
    try {
        const [defRes, ftRes] = await Promise.all([
            apiClient.get(apiUrl.value),
            !isFoundryContext.value ? apiClient.get(`admin/products/${props.slug}/file-types`) : Promise.resolve({ data: [] })
        ]);
        definitions.value = defRes.data?.data || defRes.data || [];
        fileTypes.value = ftRes.data?.data || ftRes.data || [];
...
const openDefModal = (def = null) => {
    if (def) {
        ...
        defForm.isActive = !!(def.isActive ?? def.is_active ?? true);
    } else {
        defForm.id = null;
        defForm.name = '';
        defForm.description = '';
        defForm.file_type_id = props.fileTypeId || null;
...
const saveDefinition = async () => {
    try {
        const url = `${apiUrl.value}${defForm.id ? '/' + defForm.id : ''}`;
        const method = defForm.id ? 'put' : 'post';
        
        const payload = {
            name: defForm.name,
            description: defForm.description,
            file_type_id: isFoundryContext.value ? props.fileTypeId : defForm.file_type_id,
    ...
    } catch (e) { showAlert('Error', e.response?.data?.message || 'Save failed.'); }
};
...
const deleteDefinition = async (def) => {
    if (await showConfirm('Delete Workflow', `Are you sure? This will delete the workflow blueprint and all its internal steps.`)) {
        try {
            await apiClient.delete(`${apiUrl.value}/${def.id}`);
