<template>
  <!--frontend-spa\src\layouts\AppLayout.vue -->
  <div class="bg-[#F5F7F7] font-sans min-h-screen flex flex-col">
    
    <!-- Mobile Sidebar Section (Hamburger Menu) -->
    <div v-if="isMobileMenuOpen" class="xl:hidden" role="dialog" aria-modal="true">
      ...
      <div class="fixed inset-y-0 left-0 flex flex-col w-64 bg-[#242E2C] z-50">
        ...
        <nav class="flex-1 flex flex-col px-2 pb-4 space-y-1">
          ...
          <!-- == SAAS MANAGEMENT MENU (CORE ADMINS) == -->
          <div v-if="authStore.hasRole('System Admin') || authStore.hasRole('Business Admin')" class="pt-4">
            ...
            <div class="space-y-1">
                <!-- NOTE: The `/admin/users` link for System Admins now points to the Core User Management page we built -->
          ...
          <!-- == SYSTEM SETUP MENU (SYSTEM ADMIN ONLY) == -->
      ...
      <div class="flex justify-between items-center p-4 bg-[#3b5851]">
        <div class="flex items-center space-x-8">
          ...
          <nav class="hidden xl:flex items-center space-x-1">
            ...
            <!-- == SAAS MANAGEMENT MENU (CORE ADMINS) == -->
            <template v-if="authStore.hasRole('System Admin') || authStore.hasRole('Business Admin')">
              <!-- Dropdown Menu for SaaS Management -->
              <div class="relative">
                <!-- MODIFIED: Added @click listener to toggle the menu -->
                <button @click="isSaaSMenuOpen = !isSaaSMenuOpen" class="nav-link flex items-center" type="button">
                  <span>SaaS Management</span>
                  <svg xmlns="http://www.w.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" class="ml-1 h-5 w-5"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"></path></svg>
                </button>
                <!-- MODIFIED: Added v-if to show/hide the menu panel -->
            ...
            <template v-if="authStore.hasRole('System Admin')">
              <!-- Dropdown Menu for System Setup -->
              <div class="relative">
                <!-- MODIFIED: Added @click listener to toggle the menu -->
...
<div class="flex items-center space-x-6 text-white">
          <div class="flex items-center space-x-4 border-l border-gray-400 pl-6">
            ...
            <router-link to="/my-profile" title="My Profile" class="p-2 rounded-full hover:bg-white hover:bg-opacity-20 transition-colors">
              ...
              <svg xmlns="http://www.w.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
      ...
      <div class="bg-gray-100 border-b border-gray-200 px-6 py-3 flex justify-between items-center">
        <div>
           <h1 class="text-xl font-semibold text-[#242E2C]">{{ $route.meta.displayName || $route.name }}</h1>
...
.nav-link {
  @apply px-3 py-2 rounded-md text-sm font-medium text-gray-200 hover:bg-black hover:bg-opacity-10 hover:text-white transition-colors;
}
.nav-link.active {
